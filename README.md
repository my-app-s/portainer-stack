...
Почему это идеальное решение для вашей репы?Для новичка (Локальный запуск): Он просто качает репу, пишет docker compose up -d. Переменная CERT_RESOLVER пустая $\rightarrow$ Трафик не идет в Let's Encrypt $\rightarrow$ Portainer открывается на https://localhost с локальным SSL.Для продакшена: Человек разворачивает вашу репу на боевом сервере, открывает .env, пишет DOMAIN=docker.mycompany.com, CERT_RESOLVER=myresolver, ACME_EMAIL=dev@mycompany.com. И всё! Стек сам на лету генерирует боевой сертификат.