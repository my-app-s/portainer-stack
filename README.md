...
```
    ...
    - bridge_network
```
- **ports** задает параметр проброс портов для контейнера, по *${HOST_PORT:-9443}* есть возможность задать переменной `HOST_PORT` нужный порт для внешней сети или значение по умолчанию `9443`
